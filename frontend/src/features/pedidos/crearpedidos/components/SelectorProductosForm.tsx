...
import type { CartItem } from '../../general/types/Pedido';
...
export const SelectorProductosForm: React.FC<Props> = ({ 
  ...
  const elementosAfectados = useMemo(() => {
    ...
    carrito.forEach((item) => procesarItem(item.producto, Number(item.cantidad) || 1, true));
    ...
    if (Array.isArray(pedidosPendientes)) {
      pedidosPendientes.forEach((ped) => {
        ...
        detalles.forEach((det: any) => {
          ...
          if (prodCompleto) procesarItem(prodCompleto, Number(det.cantidad || 1), false);
        });
      });
    }
  ...
  const handleContinuarSiguiente = () => {
    const elementoDeficiente = elementosAfectados.find(el => el.stockActual - el.cantTotalRequerida < 0);
    ...
    <div className="w-100 font-monospace">
      ...
      >
            ...
            {mostrarDropdown && (
              ...
              >
                {productosFiltrados.length === 0 ? (
                  <div className="p-3 small text-muted text-center">No se encontraron productos</div>
                ) : (
                  productosFiltrados.map((p) => (
                    <div
                      ...
                      style={{ 
                        cursor: 'pointer',
                        backgroundColor: String(p.idProducto) === productoId ? '#27ace6' : (isDark ? '#27272a' : '#f8fafc')
                      }}
                      onMouseDown={() => {
        ...
        <div className="mb-4">
          ...
          <div style={{ height: '135px', overflowY: 'auto' }}>
            {carrito.length === 0 ? (
              <div className="text-center my-4 py-2" style={{ color: mutedText }}>No hay productos en la lista.</div>
            ) : (
              carrito.map((item, index) => (
                <div key={index} className="d-flex align-items-center mb-2 border-bottom pb-2" style={{ borderColor: isDark ? '#2d2d30' : '#e2e8f0' }}>
        ...
        <div className="mb-4">
          <label className="form-label small fw-bold d-flex align-items-center justify-content-between">
            ...
            {porcentajeDescuento > 0 && (
              <span className="badge bg-success font-monospace fs-6">¡{porcentajeDescuento}% OFF APLICADO!</span>
            )}
          </label>
          <select 
        ...
        <div style={{ height: '100px', overflowY: 'auto' }}>
          {elementosAfectados.length === 0 ? (
            <div className="text-center mt-3 small" style={{ color: mutedText }}>
              {carrito.length === 0 ? "Agregue productos arriba para ver el impacto en el stock." : "No se identificaron requerimientos de stock."}
            </div>
          ) : (
            elementosAfectados.map((item) => {
              ...
              return (
                <div key={item.key} className="d-flex align-items-center mb-2 border-bottom pb-1 small" style={{ borderColor: isDark ? '#2d2d30' : '#e2e8f0' }}>
                  <div style={{ width: '25%' }} className="fw-semibold text-truncate">
                    {item.nombre} <span className="text-muted" style={{ fontSize: '0.75rem' }}>({item.tipo})</span>
                  </div>
                  <div style={{ width: '18%', color: '#27ace6' }} className="fw-bold">{item.cantPedidoActual} {item.unidad}</div>
                  ...
                  <div style={{ width: '21%' }}>
                    <span className={`badge ${badgeClass}`}>{badgeTexto}</span>
                  </div>
                </div>
              );
            })
      ...
      {showModalStockError && insumoFaltante && (
        <div className="modal d-block" style={{ backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 1060 }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content p-4 text-white" style={{ border: '2px solid #dc3545', backgroundColor: '#18181b', borderRadius: '12px' }}>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content p-4 text-white" style={{ border: '2px solid #ffc107', backgroundColor: '#18181b', borderRadius: '12px' }}>
              ...
              <p className="small text-light">
                El stock disponible quedará dentro del límite de tolerancia considerando el margen de respaldo:
              </p>
              <div className="list-group mb-3" style={{ maxHeight: '180px', overflowY: 'auto' }}>
                {insumosCriticos.map((ins, idx) => (
                  <div key={idx} className="list-group-item bg-dark text-white border-secondary d-flex justify-content-between align-items-center">
                    ...
                    <span className="badge bg-warning text-dark">Quedarán {ins.stockResultante} {ins.unidad}</span>
                  </div>
                ))}
              </div>
        ...
        <div className="modal d-block" style={{ backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 1060 }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content p-4 text-white" style={{ border: '2px solid #ffc107', backgroundColor: '#18181b', borderRadius: '12px' }}>
              <div className="text-center mb-3">
                <i className="bi bi-exclamation-triangle-fill fs-1 text-warning"></i>
                <h5 className="fw-bold mt-2 text-warning">Maquinaria Fuera de Servicio</h5>
              </div>
