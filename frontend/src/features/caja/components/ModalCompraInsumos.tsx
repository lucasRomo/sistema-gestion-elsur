...
export const ModalCompraInsumos: React.FC<ModalCompraInsumosProps> = ({
  ...
  // Modales de alerta/validación con diseño propio
  ...
  // Manejo de valores numéricos como string para permitir borrado completo del 0
  ...
  const handleAgregarItem = () => {
    ...
    if (!esNuevoInsumo && !idInsumoSel) {
      setAvisoModal('Debe seleccionar un insumo existente.');
      return;
    }

    if (esNuevoInsumo) {
      if (!nombreNuevo.trim()) {
        setAvisoModal('Debe ingresar el nombre del nuevo insumo.');
        return;
      }
      // VALIDACIÓN SOLICITADA: Obligatoriedad de Unidad Suelta y Unidad Empaque
      if (!idUnidad || !idUnidadCompra) {
        setAvisoModal('Debe seleccionar tanto la Unidad Suelta (Consumo) como la Unidad Empaque (Compra) para registrar el insumo.');
        return;
      }
      if (isNaN(factorNum) || factorNum <= 0) {
        setAvisoModal('El factor de conversión debe ser mayor a 0.');
        return;
      }
    }

    if (isNaN(cantNum) || cantNum <= 0) {
      setAvisoModal('La cantidad de empaques debe ser mayor a 0.');
      return;
    }

    if (isNaN(precioNum) || precioNum < 0) {
      setAvisoModal('El precio unitario no puede ser negativo.');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (itemsCompra.length === 0) {
      setAvisoModal('Debe agregar al menos un insumo a la lista de compra.');
      return;
    }

    if (isNaN(montoTotalNum) || montoTotalNum <= 0) {
      setAvisoModal('El monto total debe ser mayor a 0.');
      return;
    }
  ...
  return (
    <>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="modal-footer border-top border-secondary d-flex justify-content-between align-items-center">
                ...
                <div className="d-flex gap-2">
                  ...
                  <button type="submit" className="btn px-4 text-white fw-bold" style={{ backgroundColor: '#6f42c1' }} disabled={loading}>
      ...
      {/* MODAL DE VALIDACIÓN/AVISO PROPIO */}
      {avisoModal && (
            ...
            <div className={`modal-content ${textColor} font-monospace`} style={{ backgroundColor: cardBg, border: `1px solid ${cardBorder}`, borderRadius: '12px' }}>
              <div className="modal-header border-bottom border-secondary">
                <h5 className="modal-title fw-bold text-warning d-flex align-items-center gap-2">
                  <i className="bi bi-exclamation-triangle-fill"></i> Validación de Insumo
                </h5>
