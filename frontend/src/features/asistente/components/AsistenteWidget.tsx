...
export const AsistenteWidget: React.FC<AsistenteWidgetProps> = ({ modulo }) => {
  ...
  >
    ...
    >
      <div className="fw-bold" style={{ fontSize: '0.95rem', color: textColor }}>
        Asistente de ayuda
      </div>
    ...
    <div ref={listaRef} style={{ flex: 1, overflowY: 'auto', padding: '0.85rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
      ...
      {cargando && (
        <div style={{ alignSelf: 'flex-start', color: mutedText, fontSize: '0.85rem' }}>
          Escribiendo...
        </div>
      )}

      {error && (
        <div style={{ color: '#dc3545', fontSize: '0.82rem' }}>{error}</div>
      )}
    </div>
    ...
    <div style={{ padding: '0.65rem', borderTop: `1px solid ${borderColor}`, display: 'flex', gap: '0.5rem' }}>
      <input
        ...
        disabled={cargando}
        style={{ fontSize: '0.88rem', padding: '0.4rem 0.65rem' }}
      />
      <button
        onClick={enviarMensaje}
        disabled={cargando || !input.trim()}
        className="btn"
        style={{ backgroundColor: '#8e45e0', color: '#fff', border: 'none', padding: '0.4rem 0.8rem' }}
      >
