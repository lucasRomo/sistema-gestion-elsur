...
export const ModalRegistrosArqueo: React.FC<ModalRegistrosArqueoProps> = ({ isOpen, onClose }) => {
  ...
  // NUEVO: filtros del listado
  const [filtroFecha, setFiltroFecha] = useState(''); // formato 'YYYY-MM-DD' (input type="date")
  ...
  // NUEVO: Saldo estimado del turno calculado en vivo a partir de sus movimientos
  // (mismo criterio que el "Arqueo Automático de Turno": montoInicial + ingresos - egresos)
  ...
  // NUEVO: fecha de apertura del turno en formato 'YYYY-MM-DD' (hora local), para comparar contra el <input type="date">
  ...
  // NUEVO: turnos visibles según los filtros de fecha y estado
  ...
  // NUEVO: Antepone "Nuevo Movimiento de Caja" a los movimientos manuales
  // (los que no tienen un pedido asociado, a diferencia de señas/cobros de pedidos)
  ...
  const badgeDiferencia = (turno: Turno) => {
    if (!turno.fechaCierre) {
      return <span className="badge bg-warning-subtle text-warning">En curso</span>;
    }
    const diferencia = Number(turno.diferenciaArqueo || 0);
    if (diferencia === 0) {
      return <span className="badge bg-success-subtle text-success">Arqueo Exacto</span>;
    }
    if (diferencia > 0) {
      return <span className="badge bg-info-subtle text-info">Sobrante +{formatMonto(diferencia)}</span>;
    }
    return <span className="badge bg-danger-subtle text-danger">Faltante {formatMonto(diferencia)}</span>;
  };
        ...
        <div className="modal-content text-white" style={{ backgroundColor: '#18181b', border: '1px solid #3f3f46', borderRadius: '12px' }}>
          <div className="modal-header border-bottom border-secondary">
            <h5 className="modal-title fw-bold d-flex align-items-center gap-2">
              <i className="bi bi-journal-check" style={{ color: '#daa32d' }}></i>
          ...
          <div className="modal-body py-4" style={{ minHeight: '420px' }}>
              ...
              <>
                ...
                >
                  <div className="row g-3 align-items-end">
                    <div className="col-12 col-md-8">
                      <label className="form-label small text-white-50 mb-1">Filtro por Fecha de Apertura:</label>
                      <input
                        type="date"
                    ...
                    <div className="col-8 col-md-3">
                      <label className="form-label small text-white-50 mb-1">Estado:</label>
                      <select
                ...
                {cargandoTurnos ? (
                  <div className="text-center py-5">
                    ...
                    <p className="text-white-50 m-0">Cargando turnos...</p>
                  </div>
                ) : (
                  <div
                    className="rounded-3"
                    style={{ backgroundColor: '#121214', border: '1px solid #3f3f46', overflow: 'hidden' }}
                  >
                    <div className="table-responsive" style={{ maxHeight: '440px', overflowY: 'auto' }}>
                      <table className="table table-dark table-hover m-0 align-middle text-center">
                        <thead style={{ position: 'sticky', top: 0, backgroundColor: '#1c1c1f', zIndex: 1 }}>
                          <tr className="text-white-50 border-secondary text-uppercase" style={{ fontSize: '0.78rem', letterSpacing: '0.5px' }}>
                        ...
                        <tbody>
                          {turnosFiltrados.length === 0 ? (
                            <tr><td colSpan={5} className="py-5 text-white-50">No hay turnos que coincidan con el filtro</td></tr>
                          ) : (
                            turnosFiltrados.map((turno) => (
                              <tr key={turno.idTurno} className="border-secondary" style={{ fontSize: '0.95rem' }}>
                                <td className="fw-bold" style={{ color: '#10cbd8' }}>#{turno.idTurno}</td>
                                <td>{formatFecha(turno.fechaApertura)}</td>
                                <td>{formatFecha(turno.fechaCierre)}</td>
                                <td>{badgeDiferencia(turno)}</td>
                                <td>
                                  <button
                                    className="btn btn-sm fw-semibold text-dark"
                                    style={{ backgroundColor: '#10cbd8' }}
            ...
            {vista === 'detalle' && turnoSeleccionado && (
              <>
                <button className="btn btn-sm btn-secondary mb-3" onClick={handleVolverALista}>
                ...
                <div className="row g-3 mb-4">
                  <div className="col-6 col-md-3">
                    <div className="p-3 rounded-3 h-100" style={{ backgroundColor: '#222122', border: '1px solid #3f3f46' }}>
                      <div className="text-white-50 small mb-1">Monto Inicial</div>
                      <div className="fw-bold fs-5">{formatMonto(turnoSeleccionado.montoInicial)}</div>
                    </div>
                  </div>
                  <div className="col-6 col-md-3">
                    <div className="p-3 rounded-3 h-100" style={{ backgroundColor: '#222122', border: '1px solid #3f3f46' }}>
                      <div className="text-white-50 small mb-1">Esperado (Sistema)</div>
                      <div className="fw-bold fs-5">{formatMonto(saldoEstimadoTurno)}</div>
                    </div>
                  </div>
                  <div className="col-6 col-md-3">
                    <div className="p-3 rounded-3 h-100" style={{ backgroundColor: '#222122', border: '1px solid #3f3f46' }}>
                      <div className="text-white-50 small mb-1">Real Contado</div>
                      <div className="fw-bold fs-5">{formatMonto(turnoSeleccionado.montoRealContado)}</div>
                    </div>
                  </div>
                  <div className="col-6 col-md-3">
                    <div className="p-3 rounded-3 h-100" style={{ backgroundColor: '#222122', border: '1px solid #3f3f46' }}>
                      <div className="text-white-50 small mb-1">Diferencia</div>
                ...
                {turnoSeleccionado.observaciones && (
                  <div
                    ...
                    style={{ backgroundColor: '#2a1f0d', border: '1px solid #daa32d' }}
                  >
                    <div className="fw-bold mb-1" style={{ color: '#daa32d' }}>
                      <i className="bi bi-exclamation-triangle-fill me-2"></i>
                      Observación del cierre
                    </div>
                    <p className="m-0 text-white-50 small">{turnoSeleccionado.observaciones}</p>
                  </div>
                )}

                {/* Movimientos del turno */}
                <h6 className="fw-bold text-white-50 mb-2">Movimientos registrados en el turno</h6>
                {cargandoMovimientos ? (
                  <div className="text-center py-5">
                    <div className="spinner-border text-info mb-3"></div>
                  </div>
                      ...
                      <table className="table table-dark table-hover m-0 align-middle text-center">
                        ...
                        <tbody>
                          {movimientosTurno.length === 0 ? (
                            <tr><td colSpan={5} className="py-4 text-white-50">No hay movimientos registrados en este turno</td></tr>
                          ) : (
                            movimientosTurno.map((m) => (
                              <tr key={m.id_movimiento || m.idMovimiento} className="border-secondary" style={{ fontSize: '0.95rem' }}>
                                ...
                                <td>{m.usuario?.idUsuario || m.usuario?.id_usuario || '-'}</td>
                              </tr>
                            ))
                          )}
                        </tbody>
